...
const audioKey = (courseId: number, itemId: number) => `assignments/v1/${courseId}/${itemId}.mp3`;
const audioUrl = (courseId: number, itemId: number) => `/api/assignments/audio?course_id=${courseId}&item_id=${itemId}&v=1`;
...
export async function GET(request: Request) {
  ...
  if (!courseId || !itemId) return Response.json({ error: "Invalid assignment audio." }, { status: 400 });
  const object = await getChatAudioBucket().get(audioKey(courseId, itemId));
  if (!object) return Response.json({ error: "Assignment audio not found." }, { status: 404 });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!courseId || !itemId) return Response.json({ error: "Invalid assignment." }, { status: 400 });
    ...
    if (!title || !description) return Response.json({ error: "This assignment has no readable instructions." }, { status: 400 });
    ...
    if (!response.ok) return Response.json({ error: response.status === 429 ? "xAI is busy. This assignment will retry later." : "xAI could not create this assignment recording." }, { status: 502 });
  ...
  } catch { return Response.json({ error: "The assignment recording could not be created." }, { status: 500 }); }
}
