...
function NewTeacherEmailFlow({ courses, onSent }: { courses: Course[]; onSent: () => void }) {
  ...
  return <>
    <div className="inbox-launch-row">
      ...
      <a className="canvas-inbox-text-link" href={`${CANVAS_ORIGIN}/conversations#filter=type=inbox`} target="_blank" rel="noopener noreferrer" aria-label="Open Canvas Inbox">Open in Canvas</a>
    </div>
...
function AlertsView({ ownerUsername }: { ownerUsername: string }) {
  ...
  function addRule() {
    const now = new Date();
    const oneTime = new Date(now.getTime() + 3 * 60_000);
  ...
  return <section className="alerts-view" aria-label="Custom alarms">
    ...
    {editingRule ? <div className="alert-editor-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setEditingRuleId(null); }}><section className="alert-editor-modal" role="dialog" aria-modal="true" aria-label="Edit alarm">
      ...
      <div className="alert-rule-grid">
          <label>Schedule<select value={editingRule.scheduleType} onChange={(event) => { const scheduleType = event.target.value as "recurring" | "once"; if (scheduleType === "once") { const oneTime = new Date(Date.now() + 3 * 60_000); updateRule(editingRule.id, { scheduleType, oneTimeAt: oneTime.getTime(), oneTimeLocal: localDateTimeValue(oneTime), hour: oneTime.getHours(), minute: oneTime.getMinutes() }); } else updateRule(editingRule.id, { scheduleType }); }}><option value="recurring">Recurring</option><option value="once">One time</option></select></label>
