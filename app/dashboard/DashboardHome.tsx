...
function AlertsView({ ownerUsername }: { ownerUsername: string }) {
  ...
  return <section className="alerts-view" aria-label="Custom alarms">
    ...
    {capabilities?.native ? <div className="alerts-device-status"><strong>Android connected</strong><span>{capabilities.exact && capabilities.notifications && capabilities.overlay ? "All alarm permissions enabled" : "Finish alarm setup below"}</span>{!capabilities.notifications ? <button type="button" onClick={() => bridge?.requestNotificationAccess?.()}>Allow notifications</button> : null}{!capabilities.exact ? <button type="button" onClick={() => bridge?.requestExactAlarmAccess?.()}>Allow exact alarms</button> : null}{!capabilities.overlay ? <button className="large-alert-permission" type="button" onClick={() => bridge?.requestLargeAlertAccess?.()}>Allow large pop-up alerts</button> : <strong className="large-alert-enabled">Large pop-up alerts enabled</strong>}</div> : null}
