...
test("alarms are profile-owned, native-synced, and exposed from the menu", async () => {
  ...
  assert.doesNotMatch(dashboard, /Android connected\{/);
  assert.match(dashboard, /Alarm set for \$\{when\}/);
  ...
  assert.match(dashboard, /Opening Android Alarms & reminders settings/);
